...
den_map_p <- den_map %>% filter(StudyArea %in% unique(as.character(sa_bcr$StudyAreaID) )) %>% 
  mutate(prim = pr / saa, sec = sr / saa, allroads = r/saa,
         wint = wr /saa ) %>% right_join(studyareas, by = c("StudyArea" = "StdyAID"))
...
sa_w_cost <- all_study_areas %>% 
  ...
  mutate(INLAKE = ifelse(LC18>0.5, T, F))
...
```{r}
left_join(all_study_areas, as_tibble(sa_bcr)) %>% 
  dplyr::select(-x) %>% 
  st_as_sf() %>% 
  ggplot() +
...
BASSres <- full_BASS_run(num_runs = 100, nsamples = 10, att = sa_w_cost,q = T,
                         att.sp = st_centroid(st_as_sf(sa_w_cost)), 
                         cost = sa_w_cost, HexID_ = StdyAID, return_all = T )
```



```{r}
ggplot(left_join(study_area_hexes_w_cost, BASSres$inclusionPr)) + geom_sf(aes(fill = benefit))
```

```{r,fig.width=12, fig.height=8}
ggplot(left_join(study_area_hexes_w_cost, BASSres$inclusionPr)) + geom_sf(aes(fill = ScLogCost)) +
ggplot(left_join(study_area_hexes_w_cost, BASSres$inclusionPr)) + geom_sf(aes(fill = scale_ben)) +
...
```{r}
ggplot(left_join(study_area_hexes_w_cost, BASSres$inclusionPr)) + geom_sf(aes(fill = inclpr))
```
...
z_200 <- 
z_out %>% group_by(StdyAID) %>% 
  summarize(n=n())


sampleont <- dplyr::filter(.data = left_join(sa_w_cost, BASSres$inclusionPr), StdyAID %in% z_200$StdyAID)

sb <- spbalance(SAs[[1]], study_area_hexes_w_cost)

ggplot(sampleont) + geom_sf(data = sa_w_cost,fill=NA) + 
  geom_sf(aes(fill = inclpr))


ggplot(study_area_hexes_w_cost %>% left_join(z_200)) + geom_sf(fill=NA) + 
  geom_sf(aes(fill = n/200)) +scale_fill_viridis_c() +
  labs(fill = "", title = "Proportion of 200 grts SA chosen")

grts_er <- left_join(z_out,sa_bcr, by = c("StdyAID" = "StudyAreaID") )
