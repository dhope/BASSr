...
#' @export
#'
create_study_area <- function(landscape, study_area_size, study_unit_size,
                              units, output = "small") {

  calculate_cellsize <- function(area_ha){
    2 * (sqrt(2 * area_ha / (3 * sqrt(3)))) * sqrt(3) / 2 * 100
  }
  ...
  if(units == 'm') {
    study_area_diam <- study_area_size
    hex_diam <- study_unit_size
  }
  if(units == 'km') {
    study_area_diam <- study_area_size/1000
  ...
  HexPols_lg <- sf::st_make_grid(landscape,
                                 cellsize = study_area_diam,
                                 square = F,
                                 what = 'polygons',
                                 offset = c(0,0)) %>%
    sf::st_as_sf() %>%
    dplyr::mutate(StudyAreaID = glue::glue("SA_{stringr::str_pad(1:nrow(.), width = 4, pad = 0)}"))
  ...
  HexPols <- sf::st_make_grid(landscape,
                              cellsize = hex_diam,
                              square = F,
                              what = 'polygons',
                              offset = c(0,0)) %>%
    sf::st_as_sf() %>%
    dplyr::mutate(SampleUnitID = glue::glue("ONT_Hex_{stringr::str_pad(1:nrow(.), width = 7, pad = 0)}"))

  # small_hexes <- sp::spsample(x, type = "hexagonal", cellsize = hex_diam, offset = c(0, 0)) %>%
  #   sf::as("sf") %>%  #sf::st_as_sf?
