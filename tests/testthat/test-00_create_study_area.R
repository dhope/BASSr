...
test_that("create_study_area()", {
  ...
  # Area - m2
  expect_silent(g <- create_study_area(l, hex_size = 100000, units = "m2"))
  ...
  expect_silent(g1 <- create_study_area(l, hex_size = units::set_units(100000, m^2)))
  expect_equal(g, g1)

  # Area - km2
  expect_silent(g <- create_study_area(l, hex_size = 100, units = "km2"))
  expect_s3_class(g, "sf")
  expect_equal(units::set_units(sf::st_area(g[1,]), km^2), units::set_units(100, km^2))

  expect_silent(g1 <- create_study_area(l, hex_size = units::set_units(100, km^2)))
  expect_equal(g, g1)

  # Area - ha
  expect_silent(g <- create_study_area(l, hex_size = 10000, units = "ha"))
  expect_s3_class(g, "sf")
  expect_equal(units::set_units(sf::st_area(g[1,]), km^2), units::set_units(100, km^2))

  expect_silent(g1 <- create_study_area(l, hex_size = units::set_units(10000, ha)))
  expect_equal(g, g1)

  # Linear - short diagonal
  expect_silent(g <- create_study_area(l, hex_size = 1074.56993182354, units = "m"))
  expect_s3_class(g, "sf")
  expect_equal(sf::st_area(g[1,]), units::set_units(1000000, m^2))

  expect_silent(g1 <- create_study_area(l, hex_size = units::set_units(1074.56993182354, m)))
  expect_equal(g, g1)

  expect_silent(g <- create_study_area(l, hex_size = 10.7456993182354, units = "km"))
  expect_s3_class(g, "sf")
  expect_equal(units::set_units(sf::st_area(g[1,]), km^2), units::set_units(100, km^2))

  expect_silent(g1 <- create_study_area(l, hex_size = units::set_units(10.7456993182354, km)))
  expect_equal(g, g1)

  # Linear - long diagonal
  expect_silent(g <- create_study_area(l, hex_size = 1240.8064788028, units = "m",
                                       linear_type = "long_diagonal"))
  expect_s3_class(g, "sf")
  expect_equal(sf::st_area(g[1,]), units::set_units(1000000, m^2))

  expect_silent(g <- create_study_area(l, hex_size = 12.408064788028, units = "km",
                                       linear_type = "long_diagonal"))
  expect_s3_class(g, "sf")
  expect_equal(units::set_units(sf::st_area(g[1,]), km^2), units::set_units(100, km^2))

  # Other valid units
  expect_silent(g <- create_study_area(l, units::set_units(1076391041.6710, ft^2)))
  expect_equal(units::set_units(sf::st_area(g[1,]), km^2), units::set_units(100, km^2))

  # Params
  expect_silent(g <- create_study_area(l, 100, units = "km2", hex_prefix = "testing"))
  ...
  expect_error(create_study_area(l, hex_size = 1000, units = "km2"),
  ...
  l <- sf::st_sfc(sf::st_polygon(list(cbind(c(-150,-150,-180,-180,-150),c(45,65,65,45,45)))),
                  crs = 4326)
  expect_warning(create_study_area(l, hex_size = 1000, units = "km2"),
